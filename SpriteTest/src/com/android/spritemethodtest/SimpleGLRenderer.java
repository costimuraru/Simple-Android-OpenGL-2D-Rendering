...
public class SimpleGLRenderer implements GLSurfaceView.Renderer {
	...
	private CustomColor clearColor;
	...
	public SimpleGLRenderer(Context context) {
		...
		clearColor = new CustomColor(0, 0, 0, 1); // black
	}
	...
	public void setSprites(ArrayList<GLSprite> spriteArray) {
	...
	public void drawFrame(GL10 gl) {
		if (mSprites != null) {
			...
			for (int x = 0; x < mSprites.size(); x++) {
				GLSprite sprite = mSprites.get(x);
				if (sprite == null)
					continue;

				if (sprite.dirty == true) {
					// delete old texture
					int[] textureToDelete = new int[1];
					textureToDelete[0] = sprite.getTextureName();
					gl.glDeleteTextures(1, textureToDelete, 0);

					// load texture to sprite
					loadTexture(gl, sprite);
				}
				sprite.draw(gl);
			}
	...
	private void makeCreate(GL10 gl) {
		/*
		...
		if (mSprites != null) {
			...
			if (mUseHardwareBuffers) {
				for (int x = 0; x < mSprites.size(); x++) {
					...
					mSprites.get(x).getGrid().invalidateHardwareBuffers();
				}
			}
		...
		if (mSprites != null) {
			...
			for (int x = 0; x < mSprites.size(); x++) {
				...
				if (resource != lastFreedResource) {
					textureToDelete[0] = sprite.getTextureName();
					gl.glDeleteTextures(1, textureToDelete, 0);
					sprite.setTextureName(0);
				}
				if (mUseHardwareBuffers) {
					sprite.getGrid().releaseHardwareBuffers(gl);
				}
			}
		}
	}
	...
	protected int loadBitmap(Context context, GL10 gl, int resourceId) {
		...
		return loadBitmap(context, gl, bitmap);
	}

	public void setClearColor(CustomColor color) {
		this.clearColor = color;
	}

	private void loadTexture(GL10 gl, GLSprite sprite) {

		if (sprite.getBitmap() != null && sprite.getBitmapId() > 0) {
			// check if this sprite's bitmap is already loaded as a texture
			for (GLSprite s : mSprites) {
				if (s == null)
					continue;

				int textureName = s.getTextureName();

				// if another sprite has this bitmap
				if (s.getBitmapId() == sprite.getBitmapId() && textureName > 0) {
					sprite.setTextureName(textureName);
					sprite.dirty = false;
					Log.d("SpriteMethodTest", "Reusing texture for bitmap " + s.getBitmapId());
					return;
				}
			}
		}
		...
		if (sprite.getBitmap() != null)
			lastTextureId = loadBitmap(mContext, gl, sprite.getBitmap());
		else
			lastTextureId = loadBitmap(mContext, gl, sprite.getResourceId());
